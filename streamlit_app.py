...
def dummy_image_to_url(*args, **kwargs):
    ...
    # We assume the first argument is a PIL Image.
    image = args[0]
...
class Game:
    def __init__(self):
        # Create an off-screen pygame surface.
    ...
    def handle_mouse_click(self, pos):
        ...
        if self.mode == "shop":
            ...
            if shop_panel.collidepoint(pos):
                ...
                return
            grid_rect = pygame.Rect(GRID_ORIGIN[0], GRID_ORIGIN[1],
    ...
    def sell_crop(self, grid_x, grid_y):
            ...
            for x in range(GRID_COLS):
                ...
                if tile.crop:
                    ...
                    if tile.crop.is_mature():
                        if tile.fertilizer:
                            pygame.draw.rect(self.screen, COLOR_PROGRESS_FERTILIZED_COMPLETE, (pb_x, pb_y, int(pb_width), pb_height))
                        else:
                            pygame.draw.rect(self.screen, COLOR_PROGRESS_COMPLETE, (pb_x, pb_y, int(pb_width), pb_height))
                    elif tile.fertilizer and tile.watered:
                        progress = (((tile.crop.age * 720) + ((self.current_hour * 60) + self.current_minute)) / (tile.crop.growtime * 720))
                        pygame.draw.rect(self.screen, COLOR_PROGRESS_FERTILIZED_INCOMPLETE, (pb_x, pb_y, int(pb_width * progress), pb_height))
                    elif tile.watered:
                        progress = (((tile.crop.age * 1440) + ((self.current_hour * 60) + self.current_minute)) / (tile.crop.growtime * 1440))
                        pygame.draw.rect(self.screen, COLOR_PROGRESS_FG, (pb_x, pb_y, int(pb_width * progress), pb_height))
                    else:
...
def main():
    ...
    # Convert the pygame off-screen surface to a PIL Image.
    ...
    if canvas_result.json_data is not None:
        ...
        if "objects" in data:
            ...
            if len(data["objects"]) == 0 and st.session_state.dragging:
                st.session_state.dragging = False
                game.dragged_tiles.clear()

    # Convert and display the updated game image.
    buf = io.BytesIO()
